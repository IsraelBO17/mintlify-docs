...
```mermaid
flowchart TD
    A([Renewal Trigger]) --> B[billing.charge.requested]
    B --> C{Subscriber currency}
    C -->|NGN| D[Paystack API]
    C -->|USD| E[Stripe API]
    D --> F{Charge result}
    E --> F
    F -->|Success| G[payment.processed]
    F -->|Failure| H[Temporal Retry\nmax 3 attempts]
    H -->|All retries exhausted| I[payment.failed\n7-day grace period]
    G --> J([Subscription ACTIVE])
    I --> K[Subscription PAST_DUE\nUser notified]
    K --> L{Resolved within\n7 days?}
    L -->|Yes| J
    L -->|No| M([Suspended — Access revoked])
```
...
```mermaid
flowchart TD
    A[Gross Platform Revenue\nMonth-end] --> B[Deduct platform fee\n+ processing costs]
    B --> C[Distributable Pool]
    C --> D["Per-creator share\n= pool × creator_plays ÷ total_plays"]
    D --> E{Above minimum\nthreshold?}
    E -->|NGN ≥ 5,000 / USD ≥ 10| F[Payout via Paystack / Stripe\nFX rate locked at period start]
    E -->|Below threshold| G([Rolled to next period])
```
