...
```mermaid
flowchart LR
    MEK["Master Encryption Key\nHSM only — Annual rotation"]
    KEK["Key Encryption Key\nKMS — Per content category"]
    CEK["Content Encryption Key\nKey Store DB — Per content item"]
    MEK -->|encrypts| KEK
    KEK -->|encrypts| CEK
    CEK -->|used by| SP[Shaka Packager\nCMAF CENC + CBCS]
    SP --> SEG[(Encrypted Segments\nCDN Delivery)]
```
...
sequenceDiagram
    ...
    participant KMS
    participant DRM as Multi-DRM Provider

    C->>PS: GET /api/v1/playback/token/{contentId} + JWT
    PS->>PS: Validate entitlement, region, concurrent streams
    PS-->>C: Signed license URL (TTL: 30s)
    C->>DRM: License challenge + PSSH data + signed token
    DRM->>PS: Key retrieval callback
    PS->>KS: Fetch encrypted CEK
    KS-->>PS: Encrypted CEK
    PS->>KMS: Decrypt CEK via KEK
    KMS-->>PS: Plaintext CEK
    PS-->>DRM: CEK
    DRM-->>C: DRM License
    C->>C: Decrypt segments in hardware TEE
```
